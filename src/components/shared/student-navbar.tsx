...
export default function Navbar() {
    ...
    const navLinks = [
        ...
        { name: "Akademis", path: "/student/academic" },
        { name: "IRS", path: "/student/irs" },
    ]
    ...
    const handleNavigation = (path: string) => {
        router(path)
        setIsOpen(false) // Tutup menu setelah navigasi di mobile
    }
    ...
    return (
        <div ref={navRef} className="relative z-50">
            ...
            {/* Mobile Dropdown (Tampil jika isOpen == true di layar < md) */}
            {isOpen && (
                <div className="absolute right-0 top-full mt-2 w-48 bg-card border border-border shadow-lg rounded-md p-2 flex flex-col gap-2 md:hidden">
                    {navLinks.map((link) => (
                        <button
                            key={link.name}
                            onClick={() => handleNavigation(link.path)}
